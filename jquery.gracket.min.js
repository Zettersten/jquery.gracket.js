...
(Math.pow(2,b)-1)*(d/2):(Math.pow(2,b)-1)*d})},labels:function(a,d){var b,c=a.length,f,h=0;for(b=0;b<c;b++)f=0===b?d.padding+h:d.padding+h+d.right*b,g("<h5 />",{html:d.labels.length?d.labels[b]:"Round "+(b+1),"class":d["class"]}).css({position:"absolute",left:f,width:d.width}).prependTo(i),h+=t[b]},canvas:{resize:function(a){a=document.getElementById(a.canvasId);a.height=i.innerHeight();a.width=i.innerWidth();g(a).css({height:i.innerHeight(),width:i.innerWidth(),zIndex:1,pointerEvents:"none"})},draw:function(a,
